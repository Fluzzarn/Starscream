...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public class WebSocketServer: Server, ConnectionDelegate {
        ...
        public func start(address: String, port: UInt16) -> Error? {
            ...
            guard let listener = try? NWListener(using: parameters, on: p) else {
                return WSError(
                    type: .serverError,
                    message: "unable to start the listener at: \(address):\(port)", code: 0)
            }
        ...
        FrameCollectorDelegate, TransportEventClient
    {
        ...
        public func connectionChanged(state: ConnectionState) {
            ...
            case .failed(let error):
                print(
                    "server connection error: \(error ?? WSError(type: .protocolError, message: "default error, no extra data", code: 0))"
                )  //handleError(error)
            case .viability(_):
                break
            ...
            case .peerClosed:
                delegate?.didReceive(
                    event: .disconnected(
                        self, "Connection closed by peer",
                        UInt16(FrameOpCode.connectionClose.rawValue)))
            }
        }

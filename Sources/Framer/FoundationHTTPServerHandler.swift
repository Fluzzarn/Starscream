...
public class FoundationHTTPServerHandler: HTTPServerHandler {
    ...
    public func createResponse(headers: [String: String]) -> Data {
            ...
            let response = CFHTTPMessageCreateResponse(
                kCFAllocatorDefault, HTTPWSHeader.switchProtocolCode,
                nil, kCFHTTPVersion1_1
            ).takeRetainedValue()
            ...
            guard let cfData = CFHTTPMessageCopySerializedMessage(response)?.takeRetainedValue()
            else {
                return Data()
            }

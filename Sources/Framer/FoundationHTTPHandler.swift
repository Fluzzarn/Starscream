    ...
    public class FoundationHTTPHandler: HTTPHandler {
        ...
        public func convert(request: URLRequest) -> Data {
            let msg = CFHTTPMessageCreateRequest(
                kCFAllocatorDefault, request.httpMethod! as CFString,
                request.url! as CFURL, kCFHTTPVersion1_1
            ).takeRetainedValue()
        ...
        func parseContent(data: Data) -> Bool {
            ...
            if let cfHeaders = CFHTTPMessageCopyAllHeaderFields(response) {
                ...
                if code != HTTPWSHeader.switchProtocolCode {
                    delegate?.didReceiveHTTP(
                        event: .failure(HTTPUpgradeError.notAnUpgrade(code, headers)))
                    return true
                }

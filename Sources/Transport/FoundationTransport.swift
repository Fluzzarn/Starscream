...
public class FoundationTransport: NSObject, Transport, StreamDelegate {
    ...
    public func connect(
        url: URL, timeout: Double = 10, certificatePinning: CertificatePinning? = nil
    ) {
    ...
    private func getSecurityData() -> (SecTrust?, String?) {
            ...
            let trust =
                outputStream.property(forKey: kCFStreamPropertySSLPeerTrust as Stream.PropertyKey)
                as! SecTrust?
            var domain =
                outputStream.property(forKey: kCFStreamSSLPeerName as Stream.PropertyKey)
                as! String?
                    ...
                    as! SSLContext?
            {
                ...
                let _ = peerName.withUnsafeMutableBytes {
                    (peerNamePtr: UnsafeMutablePointer<Int8>) in
